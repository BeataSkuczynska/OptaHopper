...
def get_parsed_tokens(sentence):
    id = 0
    parents_s, rels_s, conll_s = [], [], []
    ...
    return parents_s, rels_s, conll_s
...
def parse(args):
    ...
    with open(os.path.join(args.output, "sentences.txt"), "w") as sents, \
            open(os.path.join(args.output, "parents.txt"), "w") as parents, \
            open(os.path.join(args.output, "rels.txt"), "w") as rels, \
            open(os.path.join(args.output, "to_crf.conll"), "w") as conll:
        for paragraph in parsed_sents['paragraphs']:
            for sentence in paragraph['sentences']:
                parents_s, rels_s, conll_s = get_parsed_tokens(sentence)
